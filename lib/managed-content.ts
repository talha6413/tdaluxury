...
export async function getManagedCampaigns(fallback: ManagedCampaign[]) {
  const client = serverClient(); if (!client) return fallback;
...
export async function getManagedFeaturedServices(fallback: ManagedService[]) {
  ...
  if (error || !data?.length) return fallback;
  return data.map((row) => ({ id: row.id, title: row.title, subtitle: row.short_description || row.description, meta: row.category, duration: row.duration, price: row.price_text, href: row.slug, image: realImage(row.image_url, getServiceImage(String(row.slug))), imagePosition: row.image_position || "center center" }));
}
export async function getManagedFaqGroups(fallback: ManagedFaqGroup[]) {
  ...
  for (const row of data) { const category = String(row.category || "Genel"); groups.set(category, [...(groups.get(category) ?? []), [String(row.title), String(row.answer)]]); }
...
export async function getManagedResults(fallback: ManagedResult[]) {
  ...
  if (error || !data?.length) return fallback;
  return data.map((row) => ({ id: row.id, title: String(row.title), category: String(row.category || "Sonuç"), image: `url(${realImage(row.before_image_url || row.after_image_url, "/images/real/dudak-oncesi-sonrasi.webp")})`, description: String(row.description || ""), note: String(row.disclaimer || "Sonuçlar kişiden kişiye değişebilir.") }));
}
export async function getManagedPageContent(pageKey: string) {
  const client = serverClient(); if (!client) return null;
  const { data, error } = await client.from("page_content").select("*").eq("page_key", pageKey).eq("published", true).maybeSingle();
  if (error || !data) return null;
  return { title: String(data.title), pageKey: String(data.page_key), eyebrow: String(data.eyebrow || ""), description: String(data.description || ""), buttonText: String(data.button_text || ""), buttonUrl: String(data.button_url || ""), image: realImage(data.image_url, "/images/real/salon-03.webp"), imagePosition: String(data.image_position || "center center"), seoTitle: String(data.seo_title || ""), seoDescription: String(data.seo_description || "") } satisfies ManagedPageContent;
}
export async function getManagedSiteSettings(): Promise<ManagedSiteSettings | null> {
  ...
  return {
    ...
    openingHours: normalizeOpeningHours(data.opening_hours ?? defaultOpeningHours),
    email: String(data.email || ""),
    logoMain: String(data.logo_main || "TDA"),
    logoSub: String(data.logo_sub || "LUXURY"),
    headerCtaText: String(data.header_cta_text || "RANDEVU AL"),
    headerCtaUrl: String(data.header_cta_url || "/randevu"),
    headerCtaTargetBlank: Boolean(data.header_cta_target_blank),
    footerDescription: String(data.footer_description || ""),
    footerCopyright: String(data.footer_copyright || "TDA Luxury. Tüm hakları saklıdır."),
...
export async function getManagedBlogPost(slug: string) {
  ...
  if (client) { const { data } = await client.from("blog_posts").select("*").eq("slug", slug).eq("published", true).maybeSingle(); if (data) return mapBlogRow(data); }
